...
final class Wrapper1_20_R3 implements Wrapper {
    ...
    public static net.minecraft.world.item.enchantment.Enchantment toNMS(Enchantment enchantment) {
        return CraftEnchantment.bukkitToMinecraft(enchantment);
    }
        ...
        return switch (trigger.getKey().getKey()) {
            ...
            case "brewed_potion" -> {
                ...
                yield CriteriaTriggers.BREWED_POTION.createCriterion(
                        new BrewedPotionTrigger.TriggerInstance(Optional.of(ContextAwarePredicate.create()), Optional.of(Holder.direct(toNMS(potion))))
                );
            }
            ...
            case "enter_block" -> {
                ...
                yield CriteriaTriggers.ENTER_BLOCK.createCriterion(
                        new EnterBlockTrigger.TriggerInstance(Optional.empty(), Optional.of(Holder.direct(toNMS(block))), toNMSP(state))
                );
            }
            ...
            case "inventory_changed" -> {
                ...
                if (items != null)
                    predicates.addAll(items.stream()
                            ...
                            .map(Wrapper1_20_R3::toNMSP)
                            .toList());

                yield CriteriaTriggers.INVENTORY_CHANGED.createCriterion(
                        new InventoryChangeTrigger.TriggerInstance(Optional.of(ContextAwarePredicate.create()), new InventoryChangeTrigger.TriggerInstance.Slots(toIntRange(occupiedSlots), toIntRange(fullSlots), toIntRange(emptySlots)), predicates)
                );
            }
            ...
            case "slide_down_block" -> {
                ...
                yield CriteriaTriggers.HONEY_BLOCK_SLIDE.createCriterion(
                        new SlideDownBlockTrigger.TriggerInstance(Optional.of(ContextAwarePredicate.create()), Optional.of(Holder.direct(toNMS(block))), toNMSP(state))
                );
            }
    ...
    public static ACriteria fromNMS(Criterion<?> t) {
        ...
        return new ACriteria(switch (BuiltInRegistries.TRIGGER_TYPES.getKey(t.trigger()).getPath()) {
    ...
    public static net.minecraft.advancements.AdvancementHolder toNMS(Advancement a) {
        ...
        AdvancementType frame = Arrays.stream(AdvancementType.values()).filter(f -> f.getSerializedName().equalsIgnoreCase(display.getFrame().name())).findFirst().orElse(AdvancementType.TASK);
    ...
    public static Advancement fromNMS(net.minecraft.advancements.AdvancementHolder holder) {
        ...
        NMSDisplay1_20_R3 display = new NMSDisplay1_20_R3(nmsDisplay);
    ...
    public void removeAdvancement(Player p, Set<NamespacedKey> key) {
        ServerPlayer sp = toNMS(p);
        Set<ResourceLocation> removed = key.stream().map(Wrapper1_20_R3::toNMS).collect(Collectors.toSet());
