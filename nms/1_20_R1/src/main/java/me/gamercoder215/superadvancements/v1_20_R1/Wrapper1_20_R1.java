...
final class Wrapper1_20_R1 implements Wrapper {
    ...
    public static ItemPredicate.Builder toNMS(ATriggerPredicate.Item predicate) {
        ...
        predicate.getEnchantments().stream().map(Wrapper1_20_R1::toNMS).forEach(builder::hasEnchantment);
        predicate.getStoredEnchantments().stream().map(Wrapper1_20_R1::toNMS).forEach(builder::hasStoredEnchantment);
    ...
    public static CriterionTriggerInstance toNMS(ATrigger trigger) {
        ...
        return switch (trigger.getKey().getKey()) {
            ...
            case "brewed_potion" -> {
                ...
                yield new BrewedPotionTrigger.TriggerInstance(ContextAwarePredicate.ANY, toNMS(potion));
            }
            ...
            case "cured_zombie_villager" -> {
                ...
                yield new CuredZombieVillagerTrigger.TriggerInstance(ContextAwarePredicate.ANY, wrap(toNMSP(zombie)), wrap(toNMSP(villager)));
            }
            case "enchanted_item" -> {
                ATriggerPredicate.Item item = (ATriggerPredicate.Item) c.get("item");
                Range levels = (Range) c.get("levels");
                yield new EnchantedItemTrigger.TriggerInstance(ContextAwarePredicate.ANY, toNMSP(item), toIntRange(levels));
            }
            case "enter_block" -> {
                Material block = (Material) c.get("block");
                BlockState state = (BlockState) c.get("state");
                yield new EnterBlockTrigger.TriggerInstance(ContextAwarePredicate.ANY, toNMS(block), toNMSP(state));
            }
            ...
            case "inventory_changed" -> {
                ...
                yield new InventoryChangeTrigger.TriggerInstance(ContextAwarePredicate.ANY, toIntRange(emptySlots), toIntRange(fullSlots), toIntRange(occupiedSlots), predicates);
            }
            case "item_durability_changed" -> {
                ATriggerPredicate.Item item = (ATriggerPredicate.Item) c.get("item");
                Range delta = (Range) c.get("delta");
                Range durability = (Range) c.get("durability");
                yield new ItemDurabilityTrigger.TriggerInstance(ContextAwarePredicate.ANY, toNMSP(item), toIntRange(delta), toIntRange(durability));
            }
            ...
            case "kill_mob_near_sculk_catalyst" -> {
                ATriggerPredicate.Entity entity = (ATriggerPredicate.Entity) c.get("entity");
                DamageTag killingBlow = (DamageTag) c.get("killing_blow");
                yield new KilledTrigger.TriggerInstance(CriteriaTriggers.KILL_MOB_NEAR_SCULK_CATALYST.getId(), ContextAwarePredicate.ANY, wrap(toNMSP(entity)), toNMSP(killingBlow));
            }
            case "killed_by_crossbow" -> {
                ...
                ContextAwarePredicate[] predicates = new ContextAwarePredicate[0];
                if (victims != null)
                    predicates = victims.stream()
                            .map(Wrapper1_20_R1::toNMSP)
                            .map(Wrapper1_20_R1::wrap)
                            .toArray(ContextAwarePredicate[]::new);

                yield new KilledByCrossbowTrigger.TriggerInstance(ContextAwarePredicate.ANY, predicates, toIntRange(uniqueTypes));
            }
            case "levitation" -> {
                Range distance = (Range) c.get("distance");
                Range duration = (Range) c.get("duration");
                yield new LevitationTrigger.TriggerInstance(ContextAwarePredicate.ANY, toNMSP(distance), toIntRange(duration));
            }
            ...
            case "location" -> new PlayerTrigger.TriggerInstance(new ResourceLocation("location"), ContextAwarePredicate.ANY);
            case "nether_travel" -> {
                ATriggerPredicate.Location startPosition = (ATriggerPredicate.Location) c.get("start_position");
                Range distance = (Range) c.get("distance");
                yield new DistanceTrigger.TriggerInstance(CriteriaTriggers.NETHER_TRAVEL.getId(), ContextAwarePredicate.ANY, toNMSP(startPosition), toNMSP(distance));
            }
            case "placed_block" -> {
                Material block = (Material) c.get("block");
            ...
            case "recipe_unlocked" -> {
                ...
                yield new RecipeUnlockedTrigger.TriggerInstance(ContextAwarePredicate.ANY, toNMS(recipe));
            }
            case "ride_entity_in_lava" -> {
                ATriggerPredicate.Location startPosition = (ATriggerPredicate.Location) c.get("start_position");
                Range distance = (Range) c.get("distance");
                yield new DistanceTrigger.TriggerInstance(CriteriaTriggers.RIDE_ENTITY_IN_LAVA_TRIGGER.getId(), ContextAwarePredicate.ANY, toNMSP(startPosition), toNMSP(distance));
            }
            case "shot_crossbow" -> {
                ATriggerPredicate.Item item = (ATriggerPredicate.Item) c.get("item");
            ...
            case "slide_down_block" -> {
                Material block = (Material) c.get("block");
                BlockState state = (BlockState) c.get("state");
                yield new SlideDownBlockTrigger.TriggerInstance(ContextAwarePredicate.ANY, toNMS(block), toNMSP(state));
            }
            case "started_riding" -> new StartRidingTrigger.TriggerInstance(ContextAwarePredicate.ANY);
            case "tame_animal" -> {
                ATriggerPredicate.Entity entity = (ATriggerPredicate.Entity) c.get("entity");
            ...
            case "thrown_item_picked_up_by_entity", "thrown_item_picked_up_by_player" -> {
                ATriggerPredicate.Item item = (ATriggerPredicate.Item) c.get("item");
                ATriggerPredicate.Entity entity = (ATriggerPredicate.Entity) c.get("entity");
                yield new PickedUpItemTrigger.TriggerInstance(new ResourceLocation(trigger.getKey().getKey()), ContextAwarePredicate.ANY, toNMSP(item), wrap(toNMSP(entity)));
            }
            case "tick" -> PlayerTrigger.TriggerInstance.tick();
            case "used_ender_eye" -> {
                Range distance = (Range) c.get("distance");
                yield new UsedEnderEyeTrigger.TriggerInstance(ContextAwarePredicate.ANY, toDoubleRange(distance));
            }
            case "used_totem" -> {
                ATriggerPredicate.Item item = (ATriggerPredicate.Item) c.get("item");
            ...
            case "using_item" -> {
                ATriggerPredicate.Item item = (ATriggerPredicate.Item) c.get("item");
                yield new UsingItemTrigger.TriggerInstance(ContextAwarePredicate.ANY, toNMSP(item));
            }
            case "villager_trade" -> {
                ATriggerPredicate.Item item = (ATriggerPredicate.Item) c.get("item");
                ATriggerPredicate.Entity villager = (ATriggerPredicate.Entity) c.get("villager");
                yield new TradeTrigger.TriggerInstance(ContextAwarePredicate.ANY, wrap(toNMSP(villager)), toNMSP(item));
            }
            case "voluntary_exile" -> new PlayerTrigger.TriggerInstance(new ResourceLocation("voluntary_exile"), ContextAwarePredicate.ANY);
    ...
    public static ATriggerPredicate.Entity fromNMS(ContextAwarePredicate predicate) {
        if (predicate == null) return null;
        if (predicate == ContextAwarePredicate.ANY) return ATriggerPredicate.Entity.ANY;
    ...
    public static AReward fromNMS(AdvancementRewards rewards) {
        ...
        try {
            ...
            return new AReward(
                    experience,
                    Arrays.stream(loot).map(Wrapper1_20_R1::fromNMS).collect(Collectors.toList()),
                    Arrays.stream(rewards.getRecipes()).map(Wrapper1_20_R1::fromNMS).collect(Collectors.toList())
            );
        } catch (ReflectiveOperationException e) {
            throw new RuntimeException(e);
        }
    ...
    public static Advancement fromNMS(net.minecraft.advancements.Advancement a) {
        if (a == null) return null;
        NMSDisplay1_20_R1 display = new NMSDisplay1_20_R1(a.getDisplay());
    ...
    public void removeAdvancement(Player p, Set<NamespacedKey> key) {
        ServerPlayer sp = toNMS(p);
        Set<ResourceLocation> removed = key.stream().map(Wrapper1_20_R1::toNMS).collect(Collectors.toSet());
    ...
    public AProgress getProgress(Player p, NamespacedKey key) {
        ...
        return new AProgress1_20_R1(p, nms, sp.getAdvancements().getOrStartProgress(nms));
    }
