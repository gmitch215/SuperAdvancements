package me.gamercoder215.superadvancements.v1_19_R2;
...
final class Wrapper1_19_R2 implements Wrapper {
    ...
    public static ItemPredicate.Builder toNMS(ATriggerPredicate.Item predicate) {
        ...
        predicate.getEnchantments().stream().map(Wrapper1_19_R2::toNMS).forEach(builder::hasEnchantment);
        predicate.getStoredEnchantments().stream().map(Wrapper1_19_R2::toNMS).forEach(builder::hasStoredEnchantment);
    ...
    public static CriterionTriggerInstance toNMS(ATrigger trigger) {
        ...
        return switch (trigger.getKey().getKey()) {
            ...
            case "kill_mob_near_sculk_catalyst" -> {
                ATriggerPredicate.Entity entity = (ATriggerPredicate.Entity) c.get("entity");
                yield new KilledTrigger.TriggerInstance(CriteriaTriggers.KILL_MOB_NEAR_SCULK_CATALYST.getId(), Composite.ANY, Composite.wrap(toNMSP(entity)), DamageSourcePredicate.ANY);
            }
            ...
            case "tick" -> new PlayerTrigger.TriggerInstance(new ResourceLocation("tick"), Composite.ANY);
            case "used_ender_eye" -> {
                Range distance = (Range) c.get("distance");
    ...
    public static AReward fromNMS(AdvancementRewards rewards) {
        ...
        try {
            ...
            return new AReward(
                    experience,
                    Arrays.stream(loot).map(Wrapper1_19_R2::fromNMS).collect(Collectors.toList()),
                    Arrays.stream(rewards.getRecipes()).map(Wrapper1_19_R2::fromNMS).collect(Collectors.toList())
            );
        } catch (ReflectiveOperationException e) {
            throw new RuntimeException(e);
        }
    ...
    public static Advancement fromNMS(net.minecraft.advancements.Advancement a) {
        if (a == null) return null;
        NMSDisplay1_19_R2 display = new NMSDisplay1_19_R2(a.getDisplay());
    ...
    public void removeAdvancement(Player p, Set<NamespacedKey> key) {
        ServerPlayer sp = toNMS(p);
        Set<ResourceLocation> removed = key.stream().map(Wrapper1_19_R2::toNMS).collect(Collectors.toSet());
    ...
    public AProgress getProgress(Player p, NamespacedKey key) {
        ...
        return new AProgress1_19_R2(p, nms, sp.getAdvancements().getOrStartProgress(nms));
    }
