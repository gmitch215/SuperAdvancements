...
final class Wrapper1_19_R1 implements Wrapper {
    ...
    public static ItemPredicate.Builder toNMS(ATriggerPredicate.Item predicate) {
        ...
        predicate.getEnchantments().stream().map(Wrapper1_19_R1::toNMS).forEach(builder::hasEnchantment);
        predicate.getStoredEnchantments().stream().map(Wrapper1_19_R1::toNMS).forEach(builder::hasStoredEnchantment);
    ...
    public static AReward fromNMS(AdvancementRewards rewards) {
        ...
        try {
            ...
            return new AReward(
                    experience,
                    Arrays.stream(loot).map(Wrapper1_19_R1::fromNMS).collect(Collectors.toList()),
                    Arrays.stream(rewards.getRecipes()).map(Wrapper1_19_R1::fromNMS).collect(Collectors.toList())
            );
        } catch (ReflectiveOperationException e) {
            throw new RuntimeException(e);
        }
    ...
    public static Advancement fromNMS(net.minecraft.advancements.Advancement a) {
        if (a == null) return null;
        NMSDisplay1_19_R1 display = new NMSDisplay1_19_R1(a.getDisplay());
    ...
    public void removeAdvancement(Player p, Set<NamespacedKey> key) {
        ServerPlayer sp = toNMS(p);
        Set<ResourceLocation> removed = key.stream().map(Wrapper1_19_R1::toNMS).collect(Collectors.toSet());
    ...
    public AProgress getProgress(Player p, NamespacedKey key) {
        ...
        return new AProgress1_19_R1(p, nms, sp.getAdvancements().getOrStartProgress(nms));
    }
